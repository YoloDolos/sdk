...
import { Token__factory, Market__factory, Media__factory, App__factory, Market } from '../types'
...
describe('Media', () => {
  ...
  async function deploy() {
    const token = await (await new Token__factory(deployerWallet).deploy()).deployed()
    tokenAddress = token.address

    const market = await (await new Market__factory(deployerWallet).deploy()).deployed()
    marketAddress = market.address

    const media = await (await new Media__factory(deployerWallet).deploy('ANML', 'Crypto')).deployed()
    mediaAddress = media.address

    const keeper = await (await new App__factory(deployerWallet).deploy()).deployed()
